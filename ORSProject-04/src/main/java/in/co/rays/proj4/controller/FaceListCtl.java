...
@WebServlet(name = "FaceListCtl", urlPatterns = { "/ctl/FaceListCtl" })
public class FaceListCtl extends BaseCtl {

	@Override
	protected void preload(HttpServletRequest request, HttpServletResponse response) {
		FaceModel codeModel = new FaceModel();
		try {
			List cameraList = codeModel.list();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<FaceBean> list = model.search(bean, pageNo, pageSize);
			List<FaceBean> next = model.search(bean, pageNo + 1, pageSize);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op)) {
				...
				pageNo = 1;
				FaceBean deletebean = new FaceBean();
