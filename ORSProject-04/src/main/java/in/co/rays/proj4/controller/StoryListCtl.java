...
@WebServlet(name = "StoryListCtl", urlPatterns = { "/ctl/StoryListCtl" })
public class StoryListCtl extends BaseCtl {

	@Override
	protected void preload(HttpServletRequest request, HttpServletResponse response) {
		StoryModel codeModel = new StoryModel();
		try {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<StoryBean> list = model.search(bean, pageNo, pageSize);
			List<StoryBean> next = model.search(bean, pageNo + 1, pageSize);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op)) {
				...
				pageNo = 1;
				StoryBean deletebean = new StoryBean();
	...
	protected String getView() {
		return ORSView.STORY_LIST_VIEW;
	}
}