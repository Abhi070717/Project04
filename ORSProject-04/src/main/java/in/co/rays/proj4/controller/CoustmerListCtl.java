...
@WebServlet(name = "CoustmerListCtl", urlPatterns = { "/ctl/CoustmerListCtl" })
public class CoustmerListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			List<CoustmerBean> list = model.search(bean, pageNo, pageSize);
			List<CoustmerBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op)
				...
				if (ids != null && ids.length > 0) {
					CoustmerBean deletebean = new CoustmerBean();
					for (String id : ids) {
	...
	protected String getView() {
		return ORSView.COUSTMER_LIST_VIEW;
	}
}
