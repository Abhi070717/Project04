...
public class RuleListCtl extends BaseCtl {
	...
	protected void preload(HttpServletRequest request, HttpServletResponse response) {
		RuleModel nameModel = new RuleModel();
		try {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<RuleBean> list = model.search(bean, pageNo, pageSize);
			List<RuleBean> next = model.search(bean, pageNo + 1, pageSize);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op)) {
				...
				pageNo = 1;
				RuleBean deletebean = new RuleBean();
