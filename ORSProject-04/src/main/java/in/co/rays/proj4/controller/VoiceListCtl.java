...
@WebServlet(name = "VoiceListCtl", urlPatterns = { "/ctl/VoiceListCtl" })
public class VoiceListCtl extends BaseCtl {

	@Override
	protected void preload(HttpServletRequest request, HttpServletResponse response) {
		VoiceModel codeModel = new VoiceModel();
		try {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<VoiceBean> list = model.search(bean, pageNo, pageSize);
			List<VoiceBean> next = model.search(bean, pageNo + 1, pageSize);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op)) {
				...
				pageNo = 1;
				VoiceBean deletebean = new VoiceBean();
	...
	protected String getView() {
		return ORSView.VOICE_LIST_VIEW;
	}
}